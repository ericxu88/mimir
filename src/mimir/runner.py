...
from mimir.clients.base import Client, ClientError, CompletionResponse
...
async def _call_with_retry(
    operation: Callable[[], Awaitable[CompletionResponse]],
    semaphore: asyncio.Semaphore,
    bucket: TokenBucket,
):
    # Retry stays scoped to ClientError 429/5xx: only LLM calls raise it, so
    # command/python condition failures fall straight through to the error-row path.
...
def _condition_provenance(variant) -> tuple[str, str]:
    # What lands in the conditions table's NOT NULL prompt columns; analysis never
    # reads these (variant identity flows through variant_name), so non-LLM types
    # map their own definition onto user_template as provenance.
    if variant.type == "llm":
        return variant.system, variant.user_template
    if variant.type == "command":
        return "", json.dumps(variant.command)
...
async def run_experiment(
    ...
    *,
    ...
    # Also pre-run: a missing client or a bad python import path creates no run row.
    conditions = build_conditions(spec, client, base_dir)
    run_id = store.create_run(spec.name, spec.model_dump())
    try:
        await _execute_run(spec, store, client, items, run_id, conditions)
...
async def _execute_run(
    ...
    for variant in spec.variants:
        system_prompt, user_template = _condition_provenance(variant)
        condition_ids[variant.name] = store.add_condition(
            run_id,
    ...
    # (variant_name, item_id, sample_index) -> (sample_id, response_text, envelope)
    results: dict[tuple[str, str, int], tuple[int, str, dict[str, Any]]] = {}
    had_error = False

    judge = spec.judge
    # The LLM judge is a second stochastic actor on the same cache path as
    # conditions; non-LLM scorers (parse_float) are pure and never fetch.
    llm_judge = judge if judge is not None and judge.type == "llm" else None
    judge_template = llm_judge.resolved_prompt_template() if llm_judge is not None else ""
    ...
    async def fetch(
        ...
        key: str,
        execute: Callable[[dict[str, Any]], Awaitable[CompletionResponse]],
    ) -> tuple[dict[str, Any], bool]:
        """Cache-first envelope fetch; a hit never executes the condition."""
        ...
        inflight[key] = future
        try:
            envelope = await _fetch_uncached(payload, key, execute)
    ...
    async def _fetch_uncached(
        ...
        response = await _call_with_retry(lambda: execute(payload), semaphore, bucket)
    ...
    async def judge_execute(payload: dict[str, Any]) -> CompletionResponse:
        # An llm judge implies needs_client, so build_conditions already
        # guaranteed client is not None on this path.
        return await execute_llm_payload(client, payload)
    ...
    async def sample_unit(variant, item: dict[str, Any], index: int) -> None:
        ...
        request_json = ""
        try:
            condition = conditions[variant.name]
            ...
            envelope, hit = await fetch(payload, key, condition.execute)
        ...
        results[(variant.name, item["id"], index)] = (sample_id, response_text, envelope)
    ...
    async def judge_unit(
        ...
        try:
            payload = build_payload(
                ...
                system="",
                user=render(),
                temperature=llm_judge.temperature,
            ...
            envelope, _ = await fetch(payload, row["cache_key"], judge_execute)
    ...
    def rubric_task(variant_name: str, item: dict[str, Any], index: int):
        sample_id, text, _ = results[(variant_name, item["id"], index)]

        def render() -> str:
            return judge_template.format(**_item_fields(item), response=text)

        row = {"judge_model": llm_judge.model, "mode": "rubric", "sample_a_id": sample_id}
        return judge_unit(item, render, row, parse_rubric_score, "score", index)

    async def score_unit(variant_name: str, item: dict[str, Any], index: int) -> None:
        # Pure Scorer path (parse_float): no fetch, no provider provenance —
        # judgment rows are rubric-shaped so stats.py consumes them unchanged.
        nonlocal had_error
        sample_id, _, envelope = results[(variant_name, item["id"], index)]
        row = {
            "run_id": run_id,
            "item_id": item["id"],
            "judge_model": "parse_float",
            "mode": "rubric",
            "sample_a_id": sample_id,
            "cache_key": "",
        }
        try:
            score = await scorer(CompletionResponse.model_validate(envelope), item)
        except ValueError as exc:
            had_error = True
            store.add_judgment(**row, error=str(exc))
            return
        except Exception as exc:
            # Record, never raise: same policy as every other unit.
            had_error = True
            store.add_judgment(**row, error=f"{type(exc).__name__}: {exc}")
            return
        store.add_judgment(**row, score=score)
    ...
    if llm_judge is not None:
        async with asyncio.TaskGroup() as task_group:
            if llm_judge.mode == "pairwise":
                orders = ("ab", "ba") if llm_judge.position_swap else ("ab",)
    ...
    elif judge is not None:  # the only non-llm scorer in v1 is parse_float
        ...
        async with asyncio.TaskGroup() as task_group:
            for variant in spec.variants:
                for item in items:
                    for index in range(spec.n_samples):
                        if (variant.name, item["id"], index) in results:
                            task_group.create_task(score_unit(variant.name, item, index))
